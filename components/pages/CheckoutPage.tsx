...
// Fix: Changed JSX.Element to React.ReactNode to resolve TypeScript error.
const PaymentOption: React.FC<{name: string, icon: React.ReactNode, active: boolean, onClick: () => void}> = ({name, icon, active, onClick}) => (
    <button onClick={onClick} className={`flex-1 p-4 flex flex-col items-center justify-center gap-2 border-b-4 transition-all ${active ? 'border-brand-primary text-brand-primary' : 'border-transparent text-slate-500 hover:bg-slate-50'}`}>
        {icon}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="bg-white p-6 rounded-xl shadow-md border border-slate-100">
                  ...
                  <div className="space-y-4">
                      <div>
                          ...
                          <input required type="text" id="name" className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary focus:border-transparent transition" placeholder="e.g. John Doe"/>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                              <label htmlFor="email" className="block text-sm font-medium text-slate-600 mb-1">Email Address</label>
                              <input required type="email" id="email" className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary focus:border-transparent transition" placeholder="e.g. john.doe@example.com"/>
                          </div>
                          <div>
                              <label htmlFor="phone" className="block text-sm font-medium text-slate-600 mb-1">Phone Number</label>
                              <input required type="tel" id="phone" className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary focus:border-transparent transition" placeholder="e.g. 9876543210"/>
                          </div>
                      </div>
                  </div>
              </div>
                      ...
                      {activePayment === 'card' && (
                           <div className="space-y-4">
                              <div>
                                  ...
                                  <input className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary" placeholder="XXXX XXXX XXXX XXXX" required/>
                              </div>
                               <div className="grid grid-cols-3 gap-4">
                                  <div className="col-span-2">
                                      <label className="block text-sm font-medium text-slate-600 mb-1">Expiry Date</label>
                                      <input className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary" placeholder="MM/YY" required/>
                                  </div>
                                  <div>
                                      <label className="block text-sm font-medium text-slate-600 mb-1">CVV</label>
                                      <input className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary" placeholder="XXX" required/>
                                  </div>
                               </div>
                          </div>
                      )}
                       {activePayment === 'upi' && (
                          <div>
                               ...
                               <input className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary" placeholder="yourname@bank" required/>
                          </div>
                      )}
                      {activePayment === 'wallet' && (
                           <div className="text-center text-slate-500 py-8">Wallet options are coming soon.</div>
                      )}
                  </div>
              </div>
          </div>
          <div className="md:col-span-1">
            <div className="bg-white p-6 rounded-xl shadow-md border border-slate-100 sticky top-24">
              <h2 className="text-xl font-bold mb-4 border-b pb-2">Fare Breakdown</h2>
              <div className="space-y-2">
                  <p className="font-semibold">{bus.operator_details.name}</p>
                  <p className="text-sm text-slate-500">{bus.route_from} → {bus.route_to}</p>
                  <p className="text-sm text-slate-500">Seats: <span className="font-medium text-slate-700">{seats.map(s => s.id).join(', ')}</span></p>
              </div>
              <div className="border-t my-4"></div>
              <div className="space-y-2">
                  <div className="flex justify-between">
                      <span className="text-slate-600">Base Fare:</span>
                      <span className="font-semibold text-slate-800">₹{totalFare.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between text-lg">
                      ...
                      <span className="font-bold text-brand-primary">₹{finalAmount.toFixed(2)}</span>
                  </div>
              </div>
