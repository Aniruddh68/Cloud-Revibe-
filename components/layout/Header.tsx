...
export const Header: React.FC<HeaderProps> = ({ onNavigate, currentPage, user }) => {
    ...
    return (
        <header className="bg-white/90 backdrop-blur-xl shadow-soft sticky top-0 z-50 border-b border-gray-100">
            <div className="container mx-auto px-4 py-4 flex justify-between items-center">
                <div className="flex items-center gap-2.5 cursor-pointer group" onClick={() => onNavigate('SEARCH')}>
                    <div className="bg-gradient-to-br from-brand-primary to-brand-dark p-2 rounded-xl shadow-md group-hover:shadow-lg transition-all duration-300">
                    ...
                    <h1 className="text-2xl font-extrabold bg-gradient-to-r from-brand-primary to-brand-dark bg-clip-text text-transparent tracking-tight">Miles Travels</h1>
                </div>
                <nav className="flex items-center gap-4">
                     <button
                        onClick={() => onNavigate('SEARCH')}
                        className={`hidden sm:block px-5 py-2.5 text-sm font-semibold rounded-lg transition-all duration-300 ${
                            currentPage === 'SEARCH' ? 'bg-brand-light text-brand-primary shadow-sm' : 'text-gray-600 hover:text-brand-primary hover:bg-gray-50'
                        }`}
                    >
                        Search Buses
                    </button>
                    {user && (
                        <div className="relative" ref={menuRef}>
                            <button onClick={() => setIsMenuOpen(prev => !prev)} className="h-11 w-11 bg-gradient-to-br from-gray-50 to-gray-100 rounded-full flex items-center justify-center text-gray-600 hover:from-brand-light hover:to-brand-primary/10 hover:text-brand-primary border border-gray-200 hover:border-brand-primary/30 transition-all duration-300 shadow-sm hover:shadow-md">
                            ...
                            {isMenuOpen && (
                                <div className="absolute right-0 mt-3 w-64 bg-white rounded-xl shadow-soft shadow-glow border border-gray-100 p-2 animate-scale-in">
