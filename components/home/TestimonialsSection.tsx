...
const StarRating: React.FC<{ rating: number }> = ({ rating }) => {
    return (
        <div className="flex text-amber-400">
            {[...Array(5)].map((_, index) => (
                <svg key={index} xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 ${index < rating ? 'fill-current' : 'text-gray-300'}`} viewBox="0 0 20 20" fill="currentColor">
        ...
        <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {testimonials.map(testimonial => (
                    <div key={testimonial.testimonial_id} className="bg-gray-50 p-6 rounded-lg border border-gray-200 flex flex-col h-full">
